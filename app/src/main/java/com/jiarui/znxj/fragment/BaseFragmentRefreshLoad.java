 ...
 * @date 2016年1月27日
 */
public abstract class BaseFragmentRefreshLoad extends BaseFragment implements
...
//        Toast.makeText(mContext, "请检查网络环境是否正常", Toast.LENGTH_SHORT).show();
        disposeCache();
    }
...
//            {"status":401,"msg":"token is invalid"}
//            JSONObject mCacheJson = new JSONObject(
//                    CacheUtils.getCacheToLocalJson(mCacheUrl));
            String strjson="{\"status\":-1,\"msg\":\"网络数据获取失败\"}";
                        JSONObject mCacheJson = new JSONObject( new String(strjson.getBytes(),Constants.CHARSET));
     ...
     * 处理缓存数据的方法
     */
    private void disposeCache(JSONObject row) {
            if (mWhat == AutoListView.REFRESH) {
                clearResultData();
            }
            disposeResultData(row);
    }
...
//                String mMessage = rows.optJSONObject(
//                        InterfaceDefinition.IStatus.STATUS).getString(
//                        InterfaceDefinition.IStatus.MESSAGE);
//                Log.i(TAG, mMessage + ":" + mCode);
                if (InterfaceDefinition.IStatusCode.SUCCESS.equals(status)) {
                    // 后台响应成功
//                    JSONObject mDataObject = rows
//                            .optJSONObject(InterfaceDefinition.ICommonKey.DATA);
                    JSONObject mDataObject = rows
                            .optJSONObject(InterfaceDefinition.ICommonKey.DATA);
...
//                            CacheUtils.setCacheToLocalJson(mCacheUrl,
...
//                    notifyDataSetChanged();
                    ...
                    Log.e("其他错误","其他错误其他错误");
                    // 其他错误
//                    Toast.makeText(getActivity(), mMessage, Toast.LENGTH_SHORT)
//                            .show();
                    disposeCache(rows);
                }
            }
